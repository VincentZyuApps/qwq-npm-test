...
# 1. 初始化
npm init -y
# 2. 登录 npm（需要代理的话可以用proxychains或者环境变量）
...
> **注意**：需要先在 GitHub 仓库 Settings → Secrets and variables → Actions → New repository secret 中添加 `NPM_TOKEN`，值为 npm 网站的 `npm_...` Access Token。
...
npm version patch
# 或者手动修改版本号
# 4. 再次提交（commit message 必须以 "pub" 开头才会触发发布）
git add .
...
当 push 到 `master` or `main` 分支时，GitHub Actions 会自动检查 commit message：

- 如果 **以 `pub` 开头**（不区分大小写）→ 自动执行 `npm publish`
- 否则 → 跳过发布步骤
...
flowchart TD
    ...
    Check --> Q{"commit 以 pub 开头？"}
    Q -->|否| Done1["✅ 流程结束"]
    Q -->|是| Test["test<br>npm test"]
    Test -->|失败| Fail["❌ 流程终止"]
    Test -->|通过| Pub["publish<br>npm publish"]
    Pub --> Done2["✅ 发布完成"]
```
